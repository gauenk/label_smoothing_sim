...
from utils.base import read_pickle,write_pickle,read_ndarray,write_ndarray
...
class TwoLevelCache(object):
    ...
    def save(self,payload,filenamePrefix=None,saveType="pickle",resave=False):
        if resave is not False:
            self.payload_filename = self.load_payload_filename(None)
        else:
            self.payload_filename = self._get_save_filename(filenamePrefix,saveType)
        print(__file__,self.payload_filename)
        if osp.exists(self.cache_dir) is False:
            os.makedirs(self.cache_dir)
        if saveType == "pickle":
            write_pickle(self.payload_filename,payload)
        elif saveType == "ndarray":
            write_ndarray(self.payload_filename,payload)
        else:
    ...
    def load(self,saveType="pickle",load_information=None):
        ...
        if self.payload_filename is None:
            return None
        if saveType == "pickle":
            payload = read_pickle(self.payload_filename)
        elif saveType == "ndarray":
            payload = read_ndarray(self.payload_filename)
        else:
